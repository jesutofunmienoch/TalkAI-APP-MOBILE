...
// Type declaration for VAPI (updated with actual event names)
...
const AudioConversationScreen = () => {
  ...
  useEffect(() => {
    ...
    // Handle user speech detection using volume-level (as user-specific events are not available)
    vapi.on("volume-level", (volume: number) => {
      console.log("VAPI event: volume-level", volume);
      setIsListening(volume > 0.1); // Adjust threshold based on testing (assuming volume range 0-1)
    });
  ...
  useEffect(() => {
    if (isAssistantSpeaking || (isListening && isMicOn)) {
      // Start animation when assistant is speaking or user is speaking (and not muted)
      Animated.loop(
        Animated.sequence([
